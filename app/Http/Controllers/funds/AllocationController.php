...
class AllocationController extends ParentController
{
   ...
   public function index(Request $request)
   {
      ...
      $data['budget'] = $budget;
      $monthly = $budget ? round($budget / 12, 2) : 0;
      // $monthly = 0;
      ...
      $data['budget'] = $this->getBudget($period, $economicGroup);
      ...
      if (isset($_POST['add'])) {
         ...
         $this->validate($request, [
            ...
            'budget'               => 'required|between:0,999999999999999.99',
         ]);
         ...
         if ($confirm == TRUE) {
            ...
            return view('funds.allocation.allocation', $data);
         } else {
         ...
         if (isset($_POST['edit'])) {
            ...
            $this->validate($request, [
               'budget'             => 'required|between:0,9999999999999.99',
            ]);
            ...
            DB::table('tblbudget')->where('b_id', $id)->update(['allocationValue' => $budget,]);

            //   DB::table('tblmonthlyAllocation')->where('budgetID',$id)->where('status',0)->update([
            DB::table('tblmonthlyAllocation')->where('budgetID', $id)->update([
               'amount' =>  $newBudget,
            ]);
   ...
   public function getBudget($period, $account_type = null)
   {
      if ($account_type) {
         $list = DB::table('tblbudget')
            ->leftjoin('tblallocation_type', 'tblallocation_type.ID', '=', 'tblbudget.allocationType')
            ->leftjoin('tblcontractType', 'tblcontractType.ID', '=', 'tblbudget.economicGroupID')
            ->leftjoin('tbleconomicCode', 'tbleconomicCode.ID', '=', 'tblbudget.economicCodeID')
            ->leftjoin('tbleconomicHead', 'tbleconomicHead.ID', '=', 'tblbudget.economicHeadID')
            ->leftjoin('users', 'users.id', '=', 'tblbudget.createdby')
            ->where('Period', $period) //->where(1)
            ->where('tblbudget.economicGroupID', $account_type)
            ->select('tblbudget.*', 'tblallocation_type.*', 'tblcontractType.*', 'tbleconomicCode.*', 'tbleconomicHead.*', 'users.name')
            ->orderby('tblcontractType.ID', 'ASC')
