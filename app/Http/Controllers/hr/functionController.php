...
class functionController extends Controller
{
    ...
    public function ArrearComputationCosolidatedNew($staffid, $year, $month)
    {
	 ...
	 and `staffid`='$staffid'");
        if ($checkArrear) {
            ...
            $Earn = $Earn1 * $dif['months'] + ($Earn1 * $dif['days']) / $dif['days_of_month'];
            $Deduction = $Deduction1 * $dif['months'] + ($Deduction1 * $dif['days']) / $dif['days_of_month'];
            $basic = $basic * $dif['months'] + ($basic * $dif['days']) / $dif['days_of_month'];
            $peculiar = $peculiar * $dif['months'] + ($peculiar * $dif['days']) / $dif['days_of_month'];
            $tax = $tax * $dif['months'] + ($tax * $dif['days']) / $dif['days_of_month'];
            $pension = $pension * $dif['months'] + ($pension * $dif['days']) / $dif['days_of_month'];
            $nhf = $nhf * $dif['months'] + ($nhf * $dif['days']) / $dif['days_of_month'];
            $unionDues = $unionDues * $dif['months'] + ($unionDues * $dif['days']) / $dif['days_of_month'];
            DB::table('tblstaff_for_arrears')->where('ID', $checkArrearID)->update(array(
                'payment_status'    => 1,
                'year_payment'        => $year,
                'month_payment'        => $month,
    ...
    public function OtherEarn($staffid, $year, $month)
    {
        ...
        if ($checkCV) {
            foreach ($checkCV as $b) {
                ...
                //$amount=$b->amount;
                $amount = $this->CVRembalance($b->ID, $b->amount, $b->targetAmount, $b->recycling);
                if ($amount > 0) {
...
and exists(SELECT null FROM `tblearningDeductions` WHERE `tblearningDeductions`.`ID`=`tblstaffEarningDeduction`.`earningDeductionID` and `tblearningDeductions`.`particularID`='2')");
    ...
    public function QueryStaffReportFxter($court, $division, $department, $designation, $grade, $gender, $fromdate, $todate, $type, $orderlist)
    {
        $qorderlist = "";
        if ($orderlist == "fileNo") {
            $qorderlist = "tblper.fileNo ASC";
        }
        if ($orderlist == "grade") {
            $qorderlist = "tblper.grade DESC";
        }
        if ($orderlist == "appointment_date") {
            $qorderlist = "tblper.appointment_date ASC";
        }
        if ($orderlist == "dob") {
            $qorderlist = "tblper.dob ASC";
        }

        $qtype = " 1 ";
        if ($type != '') {
            $qtype = "`tblper`.`employee_type`='$type'";
        }

        $qdesignation = 1;
        if ($designation != '') {
            $qdesignation = " exists (select tbldesignation.designation from tbldesignation where tbldesignation.departmentID=tblper.department and tbldesignation.grade=tblper.grade and tbldesignation.id='$designation')";
        }

        $qcourt = 1;
        if ($court != '') {
            $qcourt = "`tblper`.`courtID`='$court'";
        }
        ...
        $qsection = 1;
        if ($department != '') {
            $qsection = "`tblper`.`department`='$department'";
        }

        $qgrade = 1;
        if ($grade != '') {
            $qgrade = "`tblper`.`grade`='$grade'";
        }

        $qgender = 1;
        if ($gender != '') {
            $qgender = "`tblper`.`gender`='$gender'";
        }

        $qualication = " 'nil' as qualifications";
        $timedate = "(DATE_FORMAT(`tblper`.`appointment_date`,'%Y-%m-%d') BETWEEN '$fromdate' AND '$todate')";
        ...
        tblper.fileNo,
        tblper.grade DESC,
        DATE_FORMAT(tblper.date_present_appointment, '%Y-%m-%d') ASC,
        DATE_FORMAT(tblper.appointment_date, '%Y-%m-%d') ASC
    ";

        // If a specific order is selected, prepend it to the ORDER BY clause
        if (!empty($qorderlist)) {
            $orderByClause = $qorderlist . ", " . $orderByClause;
        }

        $List = DB::select("
    SELECT DISTINCT
        tblper.*,
        CONCAT(tblper.surname, ' ', tblper.first_name, ' ', tblper.othernames) AS StaffName,
        tbldesignation.designation AS designations,
        tbldivision.division AS divisions,
        tblstates.State AS State,
        lga.lga AS LGA,
        tblmaritalStatus.marital_status AS MStatus,
        'nil' AS qualifications
    FROM tblper
    LEFT JOIN tbldesignation ON tbldesignation.id = tblper.Designation
    LEFT JOIN tbldivision ON tbldivision.divisionID = tblper.divisionID
    LEFT JOIN tblstates ON tblstates.StateID = tblper.stateID
    LEFT JOIN lga ON lga.lgaId = tblper.lgaID
    LEFT JOIN tblmaritalStatus ON tblmaritalStatus.ID = tblper.maritalstatus
    WHERE $qcourt
      AND $qdivision
      AND $qsection
      AND $qgrade
      ...
      AND $timedate
      AND $qtype
      AND tblper.employee_type <> 2
      AND tblper.isAdmin = 1
      AND tblper.fileNo <> ''
    ...
    public function QueryStaffReport($court, $division, $department, $designation, $grade, $gender, $fromdate, $todate, $type, $orderlist)
    {
        ...
        // }
        if ($orderlist == "grade") {
            $qorderlist = ", `grade` DESC, `step` DESC";
        }
        ...
        $qualication = "COALESCE((
            ...
            FROM tbleducations e
            LEFT JOIN tblqualification q ON q.ID = e.degreequalification
            WHERE e.fileNo = tblper.fileNo
        ), 'nil') AS qualifications";
        ...
        // } else {
        //     $timedate = "1"; // no date filter
        // }


        // $timedate = "(DATE_FORMAT(`appointment_date`,'%Y-%m-%d') BETWEEN '$fromdate' AND '$todate')";
        // $timedate = "((DATE_FORMAT(`appointment_date`,'%Y-%m-%d') BETWEEN '$fromdate' AND '$todate') OR `appointment_date` IS NULL)";
        $timedate = "((DATE_FORMAT(`appointment_date`, '%Y-%m-%d') BETWEEN '$fromdate' AND '$todate')
              OR `appointment_date` IS NULL
              OR `appointment_date` = '0000-00-00')";


        // ✅ Add LIMIT 1 to every subquery to prevent multiple-row errors

        $List = DB::select("
            SELECT *,
                (SELECT CONCAT(`surname`, ' ', `first_name`, ' ', `othernames`)) AS StaffName,
                (SELECT `designation` FROM `tbldesignation` WHERE `tbldesignation`.`id` = `tblper`.`Designation` LIMIT 1) AS designations,
                (SELECT `division` FROM `tbldivision` WHERE tbldivision.divisionID = tblper.divisionID LIMIT 1) AS divisions,
                (SELECT `tblstates`.`State` FROM `tblstates` WHERE `tblstates`.`StateID` = `tblper`.`stateID` LIMIT 1) AS State,
                (SELECT `lga`.`lga` FROM `lga` WHERE `lga`.`lgaId` = tblper.lgaID LIMIT 1) AS LGA,
                (SELECT `tblmaritalStatus`.`marital_status` FROM `tblmaritalStatus` WHERE `tblmaritalStatus`.`marital_status` = tblper.maritalstatus LIMIT 1) AS MStatus,
                $qualication
            FROM `tblper`
            WHERE $qcourt
              AND $qdivision
              ...
              AND $timedate
              AND $qtype
              AND (`employee_type` <> 2 OR `employee_type` IS NULL)
              AND `isAdmin` = 1
            ORDER BY
                `tblper`.`rank` DESC,
                `tblper`.`grade` DESC,
    ...
    {
        $List = DB::select("SELECT * FROM tblsole_court LIMIT 1");

        if (!empty($List)) {
            return $List[0];
        }

        // Return a fallback object to avoid undefined errors
        return (object)[
            'courtstatus' => 1,
            'courtid' => null,
            'divisionstatus' => 1,
