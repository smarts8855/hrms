...
class DueForRetirementController extends functionController
{
    ...
    public function create(Request $request)
    {
        ...
        if (isset($_POST['add'])) {
            ...
            if (!$request->filled('fileNo')) {
                return back()->with('err', 'Please select a staff before submitting.');
            }
            ...
            if (DB::table('tblstaff_for_retirement')
                ->where('staffid', '=', $request['fileNo'])->first()
            ) {
                return back()->with('err', 'This staff already retired');
            }

            $staffdetails = DB::table('tblper')->where('fileNo', $request['fileNo'])->first();

            if (!$staffdetails) {
                return back()->with('err', 'Staff not found.');
            }


            $data['staffdetails'] = $staffdetails;
            // dd($staffdetails->ID.",".$request['fileNo']);
            ...
            $insert = DB::table('tblstaff_for_retirement')->insert(array(
                'staffid'                 => $staffdetails->ID,
                'fileNo'      => $staffdetails->fileNo,
                'oldEmploymentType'      => $staffdetails->employee_type,
                'courtID'                => $staffdetails->courtID,
                'old_grade'              => $staffdetails->grade,
                'old_step'               => $staffdetails->step,
        ...
        $data['staffForRetirementList'] = DB::table('tblstaff_for_retirement')
            ->join('tblper', 'tblper.ID', '=', 'tblstaff_for_retirement.staffid')
            ->join('tbldivision', 'tbldivision.divisionID', '=', 'tblstaff_for_retirement.divisionID')
            ->orderBy('tblstaff_for_retirement.id', 'desc')
            ->when(Auth::user()->is_global != 1, function ($query) {
                return $query->where('tblstaff_for_retirement.divisionID', Auth::user()->divisionID); //checking if the user is not a global user (admin) return only people his/her division
            })
            ->get();



        return view('payroll.dueForArrears.retirement', $data);
    }


    public function approveRetirement($ID)
    {

        $userId = Auth::user()->id;

        $retirement = DB::table('tblstaff_for_retirement')->where('staffid', $ID)->first();
        if (!$retirement) {
        ...
        if ($retirement->approvedBy) {
            return response()->json(['success' => false, 'message' => 'Already approved']);
        }

        DB::table('tblstaff_for_retirement')->where('staffid', $ID)->update([
            'approvedBy' => $userId
        ]);

        DB::table('tblper')->where('ID', $ID)->update([
                'staff_status' => 0,
                'status_value' => 'retirement',
                'incremental_date'     => $retirement->due_date,
                'isClaimed' => 0,
                'isAdmin' => 0,

            ]);

        return response()->json(['success' => true, 'message' => 'Staff retirement approved successfully']);
    }




    public function getStaffDetails($fileNo)
    {
        $staff = DB::table('tblper')
            ->whereRaw('TRIM(fileNo) = ?', [trim($fileNo)])
            ->first();
    ...
    public function viewRetirementDue($id)
    {

        $data['retirementInfo'] = DB::table('tblstaff_for_retirement')
            ->join('tblper', 'tblper.ID', '=', 'tblstaff_for_retirement.staffid')
            ->join('tbldivision', 'tbldivision.divisionID', '=', 'tblstaff_for_retirement.divisionID')
            ->orderBy('tblstaff_for_retirement.id', 'desc')
            ->where('tblstaff_for_retirement.staffid', $id)
            // ->when(Auth::user()->is_global != 1, function ($query) {
            //     return $query->where('tblstaff_for_retirement.divisionID', Auth::user()->divisionID); //checking if the user is not a global user (admin) return only people his/her division
            // })
            ->first();
        ...
        return view('dueForArrears.staff_retirement_data', $data);
    }
}
