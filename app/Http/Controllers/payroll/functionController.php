...
class functionController extends Controller
{
    ...
    public function Userlists()
    {
		...
		WHERE `user_type`<>'TECHNICAL'  order by user_role.`rolename` is null ASC, user_role.`rolename` ASC");
        return $List;
    }
    ...
    public function DisplayDetail($uname, $pwd)
    {
        ...
        $displayDetail = DB::table('users')
            ...
            ->select(
                ...
                'annual_leave.id',
                'annual_leave.year',
                'annual_leave.startdate',
                'annual_leave.enddate',
                'annual_leave.nod',
                'annual_leave.statusid',
                'annual_leave.hodstatus',
                'annual_leave.hodcomment',
                'annual_leave.finalapprstatus',
                'annual_leave.finalapprcomment',
                'annual_leave.leavetype',
                'annual_leave.comment',
                'annual_leave.record_status',
                'annual_leave.reapply',
                'annual_leave.reapply_status',
                'annual_leave.datetime',
                'annual_leave.hodreply'
            )
    ...
    public function PayrolldataSingleStaff($staffID)
    {
    ...
    public function PayrollStaffParameterCon($division, $bank)
    {
        $qdivision = 1;
        if ($division != 'All') {
            $qdivision = "`tblper`.`divisionID`='$division'";
		...
		and `basicsalaryconsolidated`.`courtID`=`tblper`.`courtID` and `basicsalaryconsolidated`.`grade`=`tblper`.`grade`
		and `basicsalaryconsolidated`.`step`=`tblper`.`step` and `basicsalaryconsolidated`.`employee_type`=`tblper`.`employee_type`
 		WHERE  $qdivision and $qbank and  `staff_status`='1' and `tblper`.`employee_type`<>2 and `tblper`.`employee_type`<>5");
        return $List;
    }
    public function PayrollStaffParameterWithControlNo($division, $bank, $year)
    {
        $qdivision = 1;
        if ($division != 'All') {
            $qdivision = "`tblper`.`divisionID`='$division'";
		...
		and `basicsalaryconsolidated`.`courtID`=`tblper`.`courtID` and `basicsalaryconsolidated`.`grade`=`tblper`.`grade`
		and `basicsalaryconsolidated`.`step`=`tblper`.`step` and `basicsalaryconsolidated`.`employee_type`=`tblper`.`employee_type`
 		WHERE  $qdivision and $qbank and  `staff_status`='1' and `tblper`.`employee_type`<>2 and `tblper`.`control_year`='$year' ");
        return $List;
    }
    public function PayrollCouncilParameterCon($division, $bank)
    {
        $qdivision = 1;
        if ($division != 'All') {
            $qdivision = "`tblper`.`divisionID`='$division'";
		...
		and `basicsalaryconsolidated`.`courtID`=`tblper`.`courtID` and `basicsalaryconsolidated`.`grade`=`tblper`.`grade`
		and `basicsalaryconsolidated`.`step`=`tblper`.`step` and `basicsalaryconsolidated`.`employee_type`=`tblper`.`employee_type`
 		WHERE  $qdivision and $qbank and  `staff_status`='1' and `tblper`.`employee_type`=2");

        // $List = DB::select("
        // SELECT *, `tblper`.`ID` AS `staffid`
        // FROM `tblper`
        // JOIN `basicsalaryconsolidated`
        //     ON `basicsalaryconsolidated`.`employee_type` = `tblper`.`employee_type`
        //     AND `basicsalaryconsolidated`.`courtID` = `tblper`.`courtID`
        //     AND `basicsalaryconsolidated`.`grade` = `tblper`.`grade`
        //     AND `basicsalaryconsolidated`.`step` = `tblper`.`step`
        //     AND `basicsalaryconsolidated`.`employee_type` = `tblper`.`employee_type`
        // WHERE $qdivision
        //     AND $qbank
        //     AND `staff_status` = '1'
        //     AND `tblper`.`employee_type` IN (2, 5)
        // ");
        // dd($List);
        return $List;
    }
    ...
    public function ConfirmConsolidatedPayrollperiod($court, $division, $year, $month, $bank)
    {
        ...
        $confir = DB::Select("SELECT * FROM `tblpayment_consolidated` WHERE `courtID`='$court' and $qdivision and `year`='$year' and `month`='$month' and $qbank and `employment_type`<>2");
        if (($confir)) {
            return true;
        } else {
            return false;
    ...
    public function ConfirmConsolidatedPayrollperiodCouncil($court, $division, $year, $month, $bank)
    {
        $qdivision = 1;
        if ($division != 'All') {
            $qdivision = "`divisionID`='$division'";
    ...
    public function ConfirmCheckAuditCon($division, $year, $month)
    {
        $qdivision = 1;
        if ($division != 'All') {
            $qdivision = "`divisionID`='$division'";
        ...
        $confir = DB::Select("SELECT * FROM `tblpayment_consolidated` WHERE  $qdivision and `year`='$year' and `month`='$month' and `payment_status`>1");
        if (($confir)) {
            return true;
        } else {
            return false;
    ...
    public function ConfirmCheckLockCon($division, $year, $month)
    {
        $qdivision = 1;
        if ($division != 'All') {
            $qdivision = "`divisionID`='$division'";
        ...
        $confir = DB::Select("SELECT * FROM `tblpayment_consolidated` WHERE $qdivision and `year`='$year' and `month`='$month' and `salary_lock`=1 and  `employment_type`=2");
        if (($confir)) {
            return true;
        } else {
            return false;
    ...
    public function ConfirmCheckLock($division, $year, $month)
    {
        $qdivision = 1;
        if ($division != 'All') {
            $qdivision = "`divisionID`='$division'";
    ...
    public function ConfirmCheckAudit($division, $year, $month)
    {
        $qdivision = 1;
        if ($division != 'All') {
            $qdivision = "`divisionID`='$division'";
    ...
    public function DeleteConsolidatedPayrollperiod($division, $year, $month, $bank)
    {
        $qdivision = 1;
        if ($division != 'All') {
            $qdivision = "`divisionID`='$division'";
        ...
        $qbank = 1;
        if ($bank != 'All') {
            $qbank = " (exists(SELECT null FROM `tblper` WHERE `tblper`.`bankID`='$bank' and `tblper`.`ID`=`tblpayment_consolidated`.`staffid`))";
        }
        DB::DELETE("DELETE FROM `tblpayment_consolidated` WHERE  $qdivision and `year`='$year' and `month`='$month' and $qbank and `employment_type`<>2");
    }
    public function DeleteConsolidatedPayrollperiodCouncil($court, $division, $year, $month, $bank)
    {
        $qdivision = 1;
        if ($division != 'All') {
            $qdivision = "`divisionID`='$division'";
        ...
        DB::DELETE("DELETE FROM `tblpayment_consolidated` WHERE `courtID`='$court' and $qdivision and `year`='$year' and `month`='$month' and $qbank and `employment_type`=2");
    }


    public function DeletePayrollArrearperiod($division, $year, $month, $bank)
    {
        $qdivision = 1;
        if ($division != 'All') {
            $qdivision = "`divisionID`='$division'";
        ...
        $qbank = 1;
        if ($bank != 'All') {
            $qbank = " ( exists(SELECT null FROM `tblper` WHERE `tblper`.`bankID`='$bank' and `tblper`.`ID`=`tblarrears`.`staffid`))";
        }
        DB::DELETE("DELETE FROM `tblarrears` WHERE  $qdivision and `year`='$year' and `month`='$month' and $qbank");
    }
    public function DeletePayrollOverdueArrearperiod($division, $year, $month, $bank)
    {
        $qdivision = 1;
        if ($division != 'All') {
            $qdivision = "`divisionID`='$division'";
        ...
        $qbank = 1;
        if ($bank != 'All') {
            $qbank = " ( exists(SELECT null FROM `tblper` WHERE `tblper`.`bankID`='$bank' and `tblper`.`ID`=`tblarrears_overdue`.`staffid`))";
        }
        DB::DELETE("DELETE FROM `tblarrears_overdue` WHERE  $qdivision and `year`='$year' and `month`='$month' and $qbank");
    }


    public function DeletePayrollStaffCV($division, $year, $month, $bank)
    {
        $qdivision = 1;
        if ($division != 'All') {
            $qdivision = "`divisionID`='$division'";
        ...
        $qbank = 1;
        if ($bank != 'All') {
            $qbank = " ( exists(SELECT null FROM `tblper` WHERE `tblper`.`bankID`='$bank' and `tblper`.`ID`=`tblotherEarningDeduction`.`staffid`))";
        }
        DB::DELETE("DELETE FROM `tblotherEarningDeduction` WHERE  $qdivision and `year`='$year' and `month`='$month' and $qbank and `is_staff`=1");
    }
    public function DeletePayrollCouncilCV($court, $division, $year, $month, $bank)
    {
        $qdivision = 1;
        if ($division != 'All') {
            $qdivision = "`divisionID`='$division'";
        ...
        DB::DELETE("DELETE FROM `tblotherEarningDeduction` WHERE $qdivision and `year`='$year' and `month`='$month' and $qbank and `is_staff`=0");
    }
    ...
    public function ArrearComputation($staffid, $year, $month)
    {
	 ...
	 and `staffid`='$staffid'");
        if ($checkArrear) {
            ...
            if ($oldGrade == "0" && $OldStep == "0") {
                $oldpay = DB::select("SELECT 0 as 'amount' ,0 as 'tax',0 as 'peculiar', 0 as 'leave_bonus', 0 as 'pension', 0 as 'unionDues',0 as 'utility',0 as 'nhf',0 as 'transport',0 as 'housing' ,0 as 'servant',0 as 'meal',0 as 'driver',0 as 'furniture'");
            } else {
            ...
            $Deduction = $Deduction1 * $dif['months'] + round(($Deduction1 * $dif['days']) / $dif['days_of_month'], 2);
    ...
    public function ArrearComputationCosolidatedNew($staffid, $year, $month)
    {
        $Earn = 0;
        $Deduction = 0;
        $basic = 0;
	 ...
	 and `staffid`='$staffid'");
        if ($checkArrear) {
            ...
            $oldpay = DB::select("SELECT * FROM `basicsalaryconsolidated` WHERE `employee_type`='$oldEmploymentType' and `courtID`='$court' and `grade`='$oldGrade' and `step`='$OldStep'");
            $newpay = DB::select("SELECT * FROM `basicsalaryconsolidated` WHERE `employee_type`='$newEmploymentType' and `courtID`='$court' and `grade`='$newGrade' and `step`='$newStep'");
            if (!$oldpay) return  DB::select("select 0 as 'Earn',0 as 'Deduction',0 as 'basic',0 as 'peculiar',0 as 'peculiarFG',0 as 'tax',0 as 'pension',0 as 'nhf',0 as 'unionDues'")[0];
           ...
           DB::table('tblarrears')->insert(array(
                ...
                'oldPeculiar'  => $oldpay[0]->peculiar,
                'newPeculiar'        => $newpay[0]->peculiar,
                'oldPeculiarFG'  => $oldpay[0]->peculiarFG,
                'newPeculiarFG'        => $newpay[0]->peculiarFG,
                'oldLeave_bonus'        => $oldpay[0]->leave_bonus,
                'newLeave_bonus'  => $newpay[0]->leave_bonus,
                'oldPension'        => $oldpay[0]->pension,
                'newPension'        => $newpay[0]->pension,
            ...
            $Earn1 = ($newpay[0]->amount - $oldpay[0]->amount) + ($newpay[0]->peculiar - $oldpay[0]->peculiar) + ($newpay[0]->peculiarFG - $oldpay[0]->peculiarFG) + ($newpay[0]->leave_bonus - $oldpay[0]->leave_bonus)
                + ($newpay[0]->utility - $oldpay[0]->utility) + ($newpay[0]->transport - $oldpay[0]->transport) + ($newpay[0]->housing - $oldpay[0]->housing) + ($newpay[0]->servant - $oldpay[0]->servant) + ($newpay[0]->meal - $oldpay[0]->meal) + ($newpay[0]->driver - $oldpay[0]->driver) + ($newpay[0]->furniture - $oldpay[0]->furniture);
            $basic = ($newpay[0]->amount - $oldpay[0]->amount);
            $peculiar = ($newpay[0]->peculiar - $oldpay[0]->peculiar);
            $peculiarFG = ($newpay[0]->peculiarFG - $oldpay[0]->peculiarFG);
            $Deduction1 = ($newpay[0]->tax - $oldpay[0]->tax) + ($newpay[0]->pension - $oldpay[0]->pension) + ($newpay[0]->nhf - $oldpay[0]->nhf) + ($newpay[0]->unionDues - $oldpay[0]->unionDues);
            ...
            $activemonth = date("n", strtotime($month));
            $dif = $this->dateDiff($year . "-" . $activemonth . "-1", $checkArrear[0]->due_date);
            $Earn = $Earn1 * $dif['months'] + round(($Earn1 * $dif['days']) / $dif['days_of_month'], 2);

            $basic = $basic * $dif['months'] + round(($basic * $dif['days']) / $dif['days_of_month'], 2);
            $peculiar = $peculiar * $dif['months'] + round(($peculiar * $dif['days']) / $dif['days_of_month'], 2);
            $peculiarFG = $peculiarFG * $dif['months'] + round(($peculiarFG * $dif['days']) / $dif['days_of_month'], 2);
            $tax = $tax * $dif['months'] + round(($tax * $dif['days']) / $dif['days_of_month'], 2);
            $pension = $pension * $dif['months'] + round(($pension * $dif['days']) / $dif['days_of_month'], 2);
            $nhf = $nhf * $dif['months'] + round(($nhf * $dif['days']) / $dif['days_of_month'], 2);
            $unionDues = $unionDues * $dif['months'] + round(($unionDues * $dif['days']) / $dif['days_of_month'], 2);

            $Deduction = $tax + $pension + $nhf + $unionDues; //  $Deduction1*$dif['months']+($Deduction1*$dif['days'])/$dif['days_of_month'];
            DB::table('tblstaff_for_arrears')->where('ID', $checkArrearID)->update(array(
                'payment_status'    => 1,
                'year_payment'        => $year,
                'month_payment'        => $month,
        ...
        $arreardata =  DB::select("select $Earn as 'Earn',$Deduction as 'Deduction',$basic as 'basic',$peculiar as 'peculiar',$peculiarFG as 'peculiarFG',$tax as 'tax',$pension as 'pension',$nhf as 'nhf',$unionDues as 'unionDues'");
        return $arreardata[0];
    }

    public function ArrearComputationCosolidatedNew_18_13_2025($staffid, $year, $month)
    {
        $Earn = 0;
        $Deduction = 0;
        $basic = 0;
	 ...
	 and `staffid`='$staffid'");
        if ($checkArrear) {
            ...
            $oldpay = DB::select("SELECT * FROM `basicsalaryconsolidated` WHERE `employee_type`='$oldEmploymentType' and `courtID`='$court' and `grade`='$oldGrade' and `step`='$OldStep'");
            $newpay = DB::select("SELECT * FROM `basicsalaryconsolidated` WHERE `employee_type`='$newEmploymentType' and `courtID`='$court' and `grade`='$newGrade' and `step`='$newStep'");
            if (!$oldpay) return  DB::select("select 0 as 'Earn',0 as 'Deduction',0 as 'basic',0 as 'peculiar',0 as 'tax',0 as 'pension',0 as 'nhf',0 as 'unionDues'")[0];
        ...
        $arreardata =  DB::select("select $Earn as 'Earn',$Deduction as 'Deduction',$basic as 'basic',$peculiar as 'peculiar',$tax as 'tax',$pension as 'pension',$nhf as 'nhf',$unionDues as 'unionDues'");
        return $arreardata[0];
    }

    public function OverdueArrearComputationCosolidatedNew($staffid, $year, $month)
    {
        $Earn = 0;
        $Deduction = 0;
        $m_basic = 0;
        ...
        $m_unionDues = 0;
        $m_basicArray = [];
        $checkArrear = DB::select("SELECT * FROM `tblstaff_for_arrears_overdue` WHERE ((`month_payment`='$month' and `year_payment`='$year') or `payment_status`=0)
	 and `staffid`='$staffid'");
        foreach ($checkArrear as $over_d) {

            $checkArrearID = $over_d->ID;
            $oldGrade = $over_d->old_grade;
            $newGrade = $over_d->new_grade;
            $OldStep = $over_d->old_step;
            $newStep = $over_d->new_step;
            $oldEmploymentType = $over_d->oldEmploymentType;
            $newEmploymentType = $over_d->newEmploymentType;
            $court = $over_d->courtID;
            $division = $over_d->divisionID;
            $duedate = $over_d->due_date;

            if ($oldGrade == "0" && $OldStep == "0") {
                $oldpay = DB::select("SELECT 0 as 'amount' ,0 as 'tax',0 as 'peculiar',0 as 'peculiarFG', 0 as 'leave_bonus', 0 as 'pension', 0 as 'unionDues',0 as 'utility',0 as 'nhf',0 as 'transport',0 as 'housing' ,0 as 'servant',0 as 'meal',0 as 'driver',0 as 'furniture'");
            } else {
                $oldpay = DB::select("SELECT * FROM `basicsalaryconsolidated` WHERE `employee_type`='$oldEmploymentType' and `courtID`='$court' and `grade`='$oldGrade' and `step`='$OldStep'");
            }
            $newpay = DB::select("SELECT * FROM `basicsalaryconsolidated` WHERE `employee_type`='$newEmploymentType' and `courtID`='$court' and `grade`='$newGrade' and `step`='$newStep'");
            DB::table('tblarrears_overdue')->insert(array(
                //'fileNo'    	=> $fileNo,
                'staffid'        => $staffid,
                'courtID'        => $court,
                'divisionID'        => $division,
            ...
            $peculiar = ($newpay[0]->peculiar - $oldpay[0]->peculiar);
            $peculiarFG = ($newpay[0]->peculiarFG - $oldpay[0]->peculiarFG);
            //$leave_bonus=($newpay[0]->leave_bonus-$oldpay[0]->leave_bonus);
            //$basic=($newpay[0]->amount -$oldpay[0]->amount);
            ...
            $m_basic = $basic * $dif['months'] + round(($basic * $dif['days']) / $dif['days_of_month'], 2);
            $m_basicArray[] = $m_basic;
            $m_peculiar = $peculiar * $dif['months'] + round(($peculiar * $dif['days']) / $dif['days_of_month'], 2);
            $m_peculiarFG = $peculiarFG * $dif['months'] + round(($peculiarFG * $dif['days']) / $dif['days_of_month'], 2);
            $m_tax = $tax * $dif['months'] + round(($tax * $dif['days']) / $dif['days_of_month'], 2);
            $m_pension = $pension * $dif['months'] + round(($pension * $dif['days']) / $dif['days_of_month'], 2);
            $m_nhf = $nhf * $dif['months'] + round(($nhf * $dif['days']) / $dif['days_of_month'], 2);
            $m_unionDues = $unionDues * $dif['months'] + round(($unionDues * $dif['days']) / $dif['days_of_month'], 2);

            $Deduction = $m_tax + $m_pension + $m_nhf + $m_unionDues;

            DB::table('tblstaff_for_arrears_overdue')->where('ID', $checkArrearID)->update(array(
                'payment_status'    => 1,
                'year_payment'        => $year,
                'month_payment'        => $month,
        ...
        $total_m_basic = array_sum($m_basicArray);
        $arreardata =  DB::select("select $Earn as 'Earn',$Deduction as 'Deduction',$total_m_basic as 'basic',$m_peculiar as 'peculiar',$m_peculiarFG as 'peculiarFG',$m_tax as 'tax',$m_pension as 'pension',$m_nhf as 'nhf',$m_unionDues as 'unionDues'");
        return $arreardata[0];
    }

    public function OverdueArrearComputationCosolidatedNew_18_13_2025($staffid, $year, $month)
    {
        $Earn = 0;
        $Deduction = 0;
        $m_basic = 0;
	 ...
	 and `staffid`='$staffid'");
        foreach ($checkArrear as $over_d) {
            ...
            //dd($checkArrear[0]);
            $dif = $this->dateDiff($over_d->overdueDate, $over_d->due_date);
            //if ($staffid==348)dd($basic*($dif['months']-1)+($basic*$dif['days'])/$dif['days_of_month']);
            // 	$Earn+=$Earn1*($dif['months'])+($Earn1*$dif['days'])/$dif['days_of_month'];
            ...
            // 	//echo $peculiar*($dif['months'])+($basic*$dif['days'])/$dif['days_of_month']; ;
            // 		//echo '<br>';
    ...
    public function ArrearComputationCosolidated($staffid, $year, $month)
    {
        $Earn = 0;
        $Deduction = 0;
        $checkArrear = DB::select("SELECT * FROM `tblstaff_for_arrears` WHERE ((`month_payment`='$month' and `year_payment`='$year') or `payment_status`=0)
	 and `staffid`='$staffid'");
        if ($checkArrear) {
            ...
            $newpay = DB::select("SELECT * FROM `basicsalaryconsolidated` WHERE `employee_type`='$newEmploymentType' and `courtID`='$court' and `grade`='$newGrade' and `step`='$newStep'");
            //dd("SELECT * FROM `basicsalaryconsolidated` WHERE `employee_type`='$oldEmploymentType' and `courtID`='$court' and `grade`='$oldGrade' and `step`='$OldStep'");
            DB::table('tblarrears')->insert(array(
                //'fileNo'    	=> $fileNo,
                'staffid'        => $staffid,
                'courtID'        => $court,
    ...
    public function OtherEarn($staffid, $year, $month, $is_staff)
    {
        $checkCV = DB::select("SELECT *,(SELECT `particularID` FROM `tblcvSetup` WHERE `tblcvSetup`.`ID` =`tblstaffCV`.`cvID`) as `particularID` FROM `tblstaffCV` WHERE `staffid`='$staffid' and `status`='1'");
        if ($checkCV) {
            $staffinfo = DB::table('tblper')->where('ID', $staffid)->first();
            foreach ($checkCV as $b) {

                $amount = $this->CVRembalance($b->ID, $b->amount, floatval($b->targetAmount), $b->recycling);
                if ($amount > 0) {
                    DB::table('tblotherEarningDeduction')->insert(array(
                        ...
                        'month'            => $month,
                        'year'        => $year,
                        'divisionID'    => $staffinfo->divisionID,
        ...
        $data =  DB::select("SELECT sum(`amount`) as  Total FROM `tblotherEarningDeduction` WHERE `staffid`='$staffid' and `year`='$year' and `month`='$month' and `particularID`='1'");
        $tearn = $data[0]->Total;
        $data =  DB::select("SELECT sum(`amount`) as  Total FROM `tblotherEarningDeduction` WHERE `staffid`='$staffid' and `year`='$year' and `month`='$month' and `particularID`='2'");

        $tdeduction = $data[0]->Total;


        DB::UPDATE("UPDATE `tblstaffEarningDeduction` SET `year`='$year', `month`='$month',`status`='3' WHERE `year`='' and `month`='' and `staffid`='$staffid' and `status`=1");

        $data =  DB::select("SELECT sum(`amount`) as Total FROM `tblstaffEarningDeduction` WHERE (`month`='$month' and `year`='$year') and `status`='3' and `staffid`='$staffid'
...
and exists(SELECT null FROM `tblearningDeductions` WHERE `tblearningDeductions`.`ID`=`tblstaffEarningDeduction`.`earningDeductionID` and `tblearningDeductions`.`particularID`='2')");
	...
	,$qualication
	  FROM `tblper`
	  left join `tblemployment_type` on tblemployment_type.id = tblper.employee_type
	  WHERE $qcourt and $qdivision and $qsection and $qgrade  and $qdesignation and $qgender and $timedate and $qtype and `employee_type`<>2  and `staff_status`=1
	  ORDER BY   `tblper`.`rank` DESC ,`tblper`.`grade` DESC ,DATE_FORMAT(`tblper`.`date_present_appointment`,'%Y-%m-%d') ASC,DATE_FORMAT(`tblper`.`appointment_date`,'%Y-%m-%d') ASC");
    ...
    public function JUsticeReport($court, $division, $department, $designation, $grade, $gender, $fromdate, $todate, $type, $orderlist)
    {
	...
	,$qualication
	  FROM `tblper`
	  left join `tblemployment_type` on tblemployment_type.id = tblper.employee_type
	  WHERE $qcourt and $qdivision and $qsection and $qgrade  and $qdesignation and $qgender and $timedate and $qtype and `employee_type`=2 and `staff_status`<>2 and `rank` =2
	  ORDER BY   `tblper`.`rank` DESC ,`tblper`.`grade` DESC ,DATE_FORMAT(`tblper`.`date_present_appointment`,'%Y-%m-%d') ASC,DATE_FORMAT(`tblper`.`appointment_date`,'%Y-%m-%d') ASC");
    ...
    public function QueryJusticeReport($court, $division, $department, $designation, $grade, $gender, $fromdate, $todate, $type, $orderlist)
    {
    ...
    public function EconomicText($id)
    {
        ...
        if ($List) {
            $data = $List[0]->description . '(' . $List[0]->economicCode . ')';
        }
        return $data;
    }
    public function EarningDeductionList($id)
    {
        $list = DB::table('tblcvSetup')
            ...
            ->select('tblcvSetup.ID', 'tblcvSetup.address', 'Particular', 'rank', 'description', 'bank', 'account_name', 'account_number', 'status')
    ...
    public function RankDesignationList()
    {
		...
		*/
        $List = DB::table('tblaction_rank')
            ->join('users', 'users.username', '=', 'tblaction_rank.userid')
            ->select('tblaction_rank.id', 'tblaction_rank.code', 'tblaction_rank.description', 'tblaction_rank.userid', 'users.name as userdetails')
            ->get();
        //dd($List);
        return $List;
    }
    ...
    public function MonthCount($id, $month, $year)
    {
        ...
        if ($List) {
            return ($List[0]->mcount + ($List[0]->dcount / $List[0]->of_particular_month));
        }
        return 1;
    }
    public function StaffForArrearList($division, $year, $month, $duedate)
    {
        $qyear = 1;
        $qmonth = 1;
        $qduedate = 1;
        $qdivision = "1";
        if ($year != 'All') {
            $qyear = "`year_payment`='$year'";
        }
        if ($month != 'All') {
            $qmonth = "`month_payment`='$month'";
        }
        if ($year == '') {
            $qyear = "`year_payment` is null";
        }
        if ($month == '') {
            $qmonth = "`month_payment` is null";
        }
		...
		,(SELECT Concat(`surname`,' ',`othernames`, ' ', `first_name`) FROM `tblper` WHERE `tblper`.`ID`=`tblstaff_for_arrears`.staffid) as Names
		 FROM `tblstaff_for_arrears` WHERE $qyear and $qmonth  and $qdivision order by year_payment, ID");
    }
    ...
    public function Pensionable($staffid, $year, $month)
    {
        $List = DB::Select("SELECT IFNULL(sum(`amount`),0) as penable FROM `tblotherEarningDeduction` WHERE (exists(select null from tblcvSetup where tblcvSetup.ID=`tblotherEarningDeduction`.`CVID` and tblcvSetup.pensionable=1 and tblcvSetup.particularID=1)) and `staffid`='$staffid' and `month`='$month' and `year`='$year'");
        //dd($List[0]->Taxable);
        if ($List) return $List[0]->penable;
        return 0;
    }
