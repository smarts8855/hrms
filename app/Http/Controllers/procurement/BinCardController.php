    ...
    public function index(Request $request)
    {
        ...
        if ($selectedItemId) {
            ...
            $transactions = DB::table('items_in_stores as iis')
                ...
                ->orderBy('iis.transaction_date', 'asc')
                ->select(
                    'iis.*',
                    'cr.company_name',
            ...
            foreach ($transactions as $trans) {
                ...
                // Determine reference number based on transaction type - REMOVED remark fallback
                $referenceNumber = '';
                if ($trans->item_in > 0 && !empty($trans->srv_no)) {
                    // This is a receipt (incoming) - use SRV No
                    $referenceNumber = $trans->srv_no;
                } elseif ($trans->item_out > 0 && !empty($trans->siv_no)) {
                    // This is an issue (outgoing) - use SIV No
                    $referenceNumber = $trans->siv_no;
                } else {
                    // If no SRV/SIV, leave empty instead of using remark
                    $referenceNumber = '';
                }
                
                // Add specification to reference if available
                if (!empty($referenceNumber) && $trans->specification) {
                    $referenceNumber .= ' [' . $trans->specification . ']';
                } elseif (empty($referenceNumber) && $trans->specification) {
                    // If no reference but has specification, show just the specification
                    $referenceNumber = '[' . $trans->specification . ']';
                }
