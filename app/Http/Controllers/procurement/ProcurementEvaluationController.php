...
class ProcurementEvaluationController extends Controller
{
    ...
    public function contract01(): View
    {
        ...
        foreach ($contracts as $contract) {
            ...
            // Get filtered bids based on your conditions
            $filteredBids = DB::table('tblcontract_bidding')
                ->where('contractID', $contract->contract_detailsID)
                ->where('status', '>=', 1)
                ->where('tech_evaluation', 1)
                ->where('current_location', '>=', 1)
                ->where('recommendation', '>=', 0)
                ->get();

            // Store the filtered bids in the contract object
            $contract->filtered_bids = $filteredBids;
            $contract->filtered_bids_count = $filteredBids->count();

            // Original counts (keeping your existing ones)
    ...
    {
        // Get bids with contract details
        $bidsWithContracts = DB::table('tblcontract_bidding')
            ->join('tblcontract_details', 'tblcontract_bidding.contractID', '=', 'tblcontract_details.contract_detailsID')
            ->where('tblcontract_bidding.status', '>=', 1)
            ->where('tblcontract_bidding.tech_evaluation', 1)
            ->where('tblcontract_bidding.current_location', '>=', 1)
            ->where('tblcontract_bidding.recommendation', '>=', 0)
            ->select(
                'tblcontract_bidding.*',
        ...
        // Create a collection of contracts from the bids
        $contracts = collect();
        foreach ($bidsWithContracts as $bid) {
            $contract = new \stdClass();
            $contract->contract_detailsID = $bid->contract_detailsID;
            $contract->contract_name = $bid->contract_name;
            $contract->lot_number = $bid->lot_number;
            $contract->proposed_budget = $bid->proposed_budget;
            $contract->status = 1; // Default status
            $contract->location = 1; // Default location
            $contract->reject_comment = null;
            $contract->bids = 1; // Each contract has at least this bid
            $contract->activeBids = 1;
            $contract->disabledBids = 0;
            $contract->filtered_bids = collect([$bid]); // Add the bid as filtered_bids
            $contract->filtered_bids_count = 1;

            $contracts->push($contract);
        }

        return view('procurement.Procurement.contract')
            ->with('datas', $contracts) // This satisfies the blade's expectation
            ->with('contracts', $contracts); // This satisfies the blade's expectation
    }
    ...
    public function viewContract01($contract_id)
    {
    ...
    public function viewContract($contract_id)
    {
        ...
        // Get the threshold for this contract
        $threshold = DB::table('threshold')
            ->where('id', $contract->threshold_id)
            ->first();
        ...
        $query = DB::table('tblcontract_details')
            ...
            ->select(
                ...
                'tblcontract_details.lot_number',
                'tblcontract_details.proposed_budget',
                'tblcontract_details.threshold_id',
                'tblcontract_details.status as contractStatus',
                'tblcontractor_registration.company_name'
            );

        $query = $query->where('tblcontract_bidding.contractID', $contract_id)
            ->where('tblcontract_bidding.tech_evaluation', '=', 1)
            ->where('tblcontract_bidding.status', '>', 0)
            ->get();

        // Get the current_location from the first bidding record
        $current_location = $query->isNotEmpty() ? $query->first()->current_location : 0;
        ...
        $comments = DB::table('tblcontract_comment')->where('contractID', '=', $contract_id)
            ->join('users', 'tblcontract_comment.created_by', '=', 'users.id')
            ->select('tblcontract_comment.*', 'users.name')
            ->get();

        return view('procurement.Procurement.evaluations')
            ->with('datas', $query)
            ->with('comments', $comments)
            ->with('files', $new_file)
            ->with('contract', $contract)
            ->with('threshold', $threshold)
            ->with('current_location', $current_location)
    ...
    public function recommend01(Request $request, $contractID)
    {
    ...
    public function recommend(Request $request, $contractID)
    {
        ...
        // Get the contract details to find the threshold
        $contract = DB::table('tblcontract_details')
            ->where('contract_detailsID', $contractID)
            ->first();
        ...
        // Determine the next location based on threshold role
        $nextLocation = 0;
        $locationName = '';

        if ($threshold) {
            switch ($threshold->role) {
                case 'CR': // Chief Registrar
                    $nextLocation = 2; // Location ID for Chief Registrar
                    $locationName = 'Chief Registrar';
                    break;
                case 'DTB': // Tenders Board
                    $nextLocation = 3; // Location ID for Tenders Board
                    $locationName = 'Tenders Board';
                    break;
                case 'FJTB': // Federal Judiciary Tenders Board
                    $nextLocation = 5; // Location ID for Federal Judiciary Tenders Board
                    $locationName = 'Federal Judiciary Tenders Board';
                    break;
                default:
                    $nextLocation = 1; // Default to Director Procurement
                    $locationName = 'Director Procurement';
            }

            // Update the bidding records with the new location
            // Update all bidding records for this contract that are not disqualified
            DB::table('tblcontract_bidding')
                ->where('contractID', $contractID)
                ->where('status', '>', 0) // Only active bids
                ->update([
                    'current_location' => $nextLocation,
                    'updated_at' => date('Y-m-d H:i:s')
                ]);

            // Also update the specific recommended bid if needed
            DB::table('tblcontract_bidding')
            ...
            // Add a comment about the auto-movement
            $movementComment = new ProComment;
            $movementComment->contractID = $contractID;
            $movementComment->biddingID = $biddingID;
            $movementComment->comment_description = "System automatically moved contract to " . $locationName . " based on threshold amount of ₦" . number_format($contract->proposed_budget, 2) . " (Threshold: " . $threshold->role . " - ₦" . number_format($threshold->min, 2) . " to ₦" . number_format($threshold->max, 2) . ")";
            $movementComment->created_by = $user;
            $movementComment->status = 1;
            $movementComment->save();
        }

        $addComment = new ProComment;
        $addComment->contractID = $contractID;
        $addComment->biddingID = $biddingID;
        $addComment->comment_description = $user_name->name . " recommended " . $contractor_name->company_name . "'s bid in Financial evaluation stage. " . $comment;
        $addComment->created_by = $user;
        $addComment->status = 1;
        $addComment->save();

        // Add movement info to success message
        $successMessage = "Recommendation made successfully";
        if ($threshold) {
            $successMessage .= ". Contract automatically moved to " . $locationName . " based on threshold amount.";
        }

        return redirect('/pro-procurement/contract/' . encrypt($contractID))->with('success', $successMessage);
    }
