...
<div class="box box-default">
                        ...
                        @if($tablecontent)
                            @foreach($tablecontent as $list)
                               <tr @if($list->isrejected==1) style="background-color: red; color:#FFF;" @endif>
                                      ...
                                      <div class="dropdown">
                                          <button class="btn btn-danger btn-xs dropdown-toggle" type="button" id="dropdownMenu1" data-toggle="dropdown" aria-haspopup="true" aria-expanded="true">
                                            ...
                                            <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">                                   
                                                ...
                                                <li><a onclick="lock_unlock_voucher('{{$list->ID}}')"> Lock/Unlock Voucher </a></li>
                                            </ul>
                                        </div>
                                        </td>
                                    ...
                                    <td>{{($list->voucher_lock ? 'Locked' : 'Open')}}</td>
                                </tr>
                            @endforeach
                        @else
                            <tr>
 ...
 <!--decline modal-->
    ...
    {{ csrf_field() }}
     ...
     <!--lock/Unclock Voucher Modal-->
        <div id="lockUnclockVoucherModal" class="modal fade">
            <form class="form-horizontal" role="form" method="post" action="{{url('voucher/lock')}}">
            {{ csrf_field() }}
                <div class="modal-dialog" role="document">
                  <div class="modal-content">
                    <div class="modal-header">
                      <h4 class="modal-title">Lock/Unlock Voucher</h4>
                      <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                      </button>
                    </div>
                    <div class="modal-body"> 
                     <h5> Decide if this voucher can be moved OR cannot be moved by other units. </h5>
                        <div class="form-group" style="margin: 0 10px;">
                            <div class="col-sm-12">
                            <label class="control-label"><b>Select action</b></label>
                            </div>
                            <div class="col-sm-12">
                                    <select name="voucherAction" required class="form-control">
                                        <option value="0">Unlock(Open for movement)</option>
                                        <option value="1">Lock(Cannot be moved)</option>
                                    </select>
                            </div>
                            <input type="hidden"  id="lockVoucherID" name="voucherID">
                        </div>
                    </div>
                        <div class="modal-footer">
                            <button type="Submit" name="lockVoucher" class="btn btn-success">Continue</button>
                            <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
                        </div>
        ...
        <!--end of lock/Unclock Voucher Modal-->
...
function decline(a){ 
    ...
    //Get voucher ID unclick Event
    function lock_unlock_voucher(a){
        document.getElementById('lockVoucherID').value=a;
        $("#lockUnclockVoucherModal").modal('show')
        return false;
    }
</script>
