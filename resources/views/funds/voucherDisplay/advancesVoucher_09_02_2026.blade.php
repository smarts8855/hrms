    	...
    	<div class="printWrap">
		       <select class="type print-voucher">
		           <option value="">Select Voucher to print</option>
							...
							<option value="prem">RETIRED VOUCHER</option>
	 ...
	 <div class="col-xs-12">
		<div class="box-body">
				...
				<div align="center" style="font-weight: 100">
					Departmental No. <b>SCN/ADV/<input type="text" class="noborder" style="border:none; width:40px !important;" transid="{{$list->transID}}" id="vref" value="{{$list->vref_no}}" />/{{date('Y', strtotime($list->datePrepared))}}</b>. Checked and passed for payment at <b>Abuja</b>
				</div>
		</div>
	  </div>
	</div>
	    ...
	    @if($list->contractTypeID != 4)
	 	<div class="col-xs-3 sidetblock">
	 		...
	 		<table style="font-size: 10px; margin-left: 4px; margin-top: -25px;">
	 			...
	 			<tr>
	 				<td><div class="vertical-text v-align-3">&#8358; {{number_format(($list->totalPayment), 2, '.', ',')}}</div></td>
	 			</tr>
	 			<tr>
	...
	<table class="table table-condensed table-bordered text-center input-sm">
		...
		<tbody>
			...
			<tr>
				...
				<td><big>{{number_format(($list->totalPayment), 2, '.', ',')}}</big></td>
			</tr>
			<tr>
				<td colspan="5">
					<div class="row">
...
<!------------- Retirement Voucher ------------->
@if($list->retiredStatus == 1)
<div class="box-body" id="prem" style="background: #fff;">
<div style="margin: 0 10px;">
	<div class="row">
	 <div class="col-xs-12">
		<div class="box-body">
				<div align="center">
					<h4>
						<div class="make-bold">
                            ...
                            <span class="pull-right hidden-print"><small><span  style="color:green;">STATUS: </span>Retired</small></span>
						</div>
					</h4>
				...
				<div align="center" style="font-weight: 100">
					Departmental No. <b>SCN/ADV/{{$list->vref_no}}/{{date('Y', strtotime($list->datePrepared))}}</b>. Checked and passed for payment at <b>Abuja</b>
				</div>
		</div>
	  </div>
	</div>
	...
	<div style="margin-bottom: 2px; margin-top:20px;">
		...
		<div style="text-decoration: none;border-bottom: 2px dotted #000; margin-top:20px;">
			<strong style="font-size:18px;">Please, recieve the sum of:</strong> &nbsp;&nbsp;&nbsp; <span class="input-lg"><small>
		{{$list->retirement_remark}}
			</small></span>
		</div>
		<div style="width:100%; float:left; text-decoration: none;border-bottom: 2px dotted #000; margin-top:20px;">
		<p class="pull-right"><strong>AMOUNT GRANTED:</strong> {{number_format($list->totalPayment,2)}}</p>
		</div>
		<div class="clearfix"></div>
		<div style="width:100%; float:left;  text-decoration: none;border-bottom: 2px dotted #000; margin-top:20px;">
		<p class="pull-right"><strong>AMOUNT SPEND:</strong> {{number_format($list->amount_retired,2)}}</p>
		</div>
		<div class="clearfix"></div>
		<div style="width:100%; float:left;  text-decoration: none;border-bottom: 2px dotted #000; margin-top:20px;">
		<p class="pull-right"><strong>BALANCE:</strong> {{number_format($list->totalPayment - $list->amount_retired,2)}}</p>
		</div>
	</div><br/><br/><br/>
	<div class="clearfix"></div>
	<p style="margin:30px;">
	<strong style="font-size:18px;">N{{number_format($list->totalPayment,2)}}</strong>
	</p>
...
<div class="row">
	...
	<div class="col-md-5 pull-right">
		...
		<p><strong>SIGNATURE/MARK OF PAYER</strong></p>
	</div>
</div>
<div class="row" style="padding-top:30px;">
    <div class="col-md-5 pull-right">
	    <strong>WITNESS TO MARK </strong> --------------------------
...
$( "#vref" ).blur(function() {
  ...
  type: "post",
  data: {'transactionID':transactionID, 'vref': vref, _token:'{{csrf_token()}}'},
