 ...
 <div class="box-header with-border hidden-print">
      <h3 class="box-title">@yield('pageTitle') <span id='processing'></span></h3>
        ...
        <div class="row">
            ...
            {
            @endphp
            @include('hr.StaffDocumentation.inc.wizard')
            @php
            }
            @endphp


<form method="post" action="{{url('/staff-documentation')}}" id="form1">
    {{ csrf_field() }}
    <div class="noprint">
                    <div class="col-md-3">
                    <input type="hidden"     value="{{$fileNo}}" id="fileNo" name="fileNo">
                    <input type="hidden"     value="{{$staffID}}" id="staffID" name="staffID">
                    <label>Staff File Number</label>
                    <input type="text" list="customerid" id="numvalue"   class="form-control" placeholder="Select staff file no"  autocomplete="off" onchange="StaffSearchReload()">
                ...
                <input type="text"   class="form-control"  value="{{$StaffNames}}" placeholder="{{$StaffNames}}" disabled>

                    </div>
                    <div class="col-md-2">
                        <label>&nbsp&nbsp&nbsp</label>
                <input type="submit" onclick="return ReloadForm()"   class="form-control btn btn-success"  value="Get Staff!">
                    </div>
                    </div>
</form>
</div>
...
<div role="form">
    @if($prog == 6)
        @include('hr.StaffDocumentation.inc.basicInfo')
    @elseif($prog == 7)
    	@include('hr.StaffDocumentation.inc.contact')
    @elseif($prog == 8)
        @include('hr.StaffDocumentation.inc.placeOfBirth')
    @elseif($prog == 9)
    	@include('hr.StaffDocumentation.inc.marital')
    @elseif($prog == 10)
        @include('hr.StaffDocumentation.inc.nextOfKin')
    @elseif($prog == 11)
        @include('hr.StaffDocumentation.inc.children')
    @elseif($prog == 12)
        @include('hr.StaffDocumentation.inc.previousEmployment')
     @elseif($prog == 13)
        @include('hr.StaffDocumentation.inc.attachment')
    @elseif($prog == 14)
        @include('hr.StaffDocumentation.inc.account')
    @elseif($prog == 15)
        @include('hr.StaffDocumentation.inc.others')
    @elseif($prog == 16)
        @include('hr.StaffDocumentation.inc.previewInfo')
    @elseif($prog == 17)
        @include('hr.StaffDocumentation.inc.complete')
    @endif
</div><!--//Role-->
      ...
      $token = $("input[name='_token']").val();
        $.ajax({
           headers: {'X-CSRF-TOKEN': $token},
            type: 'POST',
            url: murl + "/staff-documentation-getLga",
            data: {'id': state},
