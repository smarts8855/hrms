...
<table class="table table-condense table-responsive" border="1" cellpadding="4" cellspacing="0">
  <tr>
    <td rowspan="2" align="center" valign="middle"><strong>FILE NO</strong></td>
    <td rowspan="2" align="center" valign="middle"><strong>NAME</strong></td>
    <td rowspan="2" align="center" valign="middle"><strong>GL</strong></td>
    <td rowspan="2" align="center" valign="middle"><strong>ST</strong></td>
    ...
    <td rowspan="2" align="center" valign="middle"><strong>NETPAY</strong></td>
  </tr>
  <tr>
    <td align="center" valign="middle"><strong>BASIC</strong></td>
    <td align="center" valign ="middle"><strong> JUSU </strong></td>


    <td align="center" valign="middle"><strong>ARREARS</strong></td>
    <td align="center" valign="middle"><strong>OTHERS</strong></td>
    <td align="center" valign="middle"><strong>TOTAL EARNING</strong></td>

    <td align="center" valign="middle"><strong>TAX</strong></td>
    <td align="center" valign="middle"><strong>PENSION</strong></td>

    <td align="center" valign="middle"><strong>NHF</strong></td>
    <td align="center" valign="middle"><strong>ARREARS</strong></td>
    <td align="center" valign="middle"><strong>OTHER/DEDUCT.</strong></td>
  ...
  @foreach ($payroll_detail as $reports)
    ...
    <tr>
      ...
      <td align="center" valign="middle"> {{ $reports->grade }}</td>
      <td align="center" valign="middle">{{$reports->step}}</td>
      <td align="right"><?php $bstotal += $reports->Bs; ?> {{number_format($reports->Bs, 2, '.', ',')}}</td>
      <td align="right" valign="middle"><?php $pectotal += $reports->PEC;?> {{ number_format($reports->PEC, 2, '.', ',') }}</td>
      @if($reports->AEarn == "")
        <td align="right"><?php $e_arrearstotal += $reports->AEarn;?>{{number_format($reports->AEarn, 2, '.', ',')}}</td>
      @else
        <td align="right"><?php $e_arrearstotal += $reports->AEarn;?><a href="{{url("/con-payrollReport/arrears/$reports->courtID/$fileNo/$reports->year/$reports->month")}}" target="_blank">{{number_format($reports->AEarn, 2, '.', ',')}}</a></td>
      @endif
      <td align="right"><?php $e_otherstotal += $reports->OEarn;?> {{number_format($reports->OEarn, 2, '.', ',')}}</td>
      <td align="right"><?php $earntotal += $reports->TEarn;?> {{number_format($reports->TEarn, 2, '.', ',')}}</td>

      <td align="right"><?php $taxtotal += $reports->TAX;?> {{number_format($reports->TAX, 2, '.', ',')}}</td>
      <td align="right"><?php $pentotal += $reports->PEN;?> {{number_format($reports->PEN, 2, '.', ',')}}</td>
      <td align="right"><?php $nhftotal += $reports->NHF;?> {{number_format($reports->NHF, 2, '.', ',')}}</td>

      @if($reports->AD == "")
        <td align="right"><?php $d_arrearstotal += $reports->AD;?> {{number_format($reports->AD, 2, '.', ',')}}</td>
      @else
        <td align="right"><?php $d_arrearstotal += $reports->AD;?>
      ...
      @endif
      <td align="right"><?php $d_othertotal += $reports->OD;?> {{number_format($reports->OD, 2, '.', ',')}}</td>

      <td align="right"><?php $deducttotal += $reports->TD;?> {{number_format($reports->TD, 2, '.', ',')}}</td>
      <td align="right"><?php $netpaytotal += $reports->NetPay;?> {{number_format($reports->NetPay, 2, '.', ',')}}</td>
    </tr>
...
<div align="center">
 ...
 <input type="submit"  value="Process" class="btn btn-success" id="btn" />
    ...
    @endif
     ...
     @if(!is_null($track))
    @if($track->sent_by === auth::user()->username && $track->sent_to != $action->code)
      <form method="post" action="{{url('/con-payroll/recall')}}" style="margin-top:10px;">
        {{ csrf_field() }}
        ...
        @endif
        <input type="submit" name="btn"  value="Recall" class="btn btn-success" />
      </form>
    @endif
    @endif
</div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        @endforeach
        <form method="post" action="{{url('/con-payroll/dfa')}}" style="margin-top:10px;">
          {{ csrf_field() }}
          <input type="hidden"  value="{{$year}}" name="year" class="btn btn-success" />
          <input type="hidden"  value="{{$month}}" name="month" class="btn btn-success" />
          <!--<input type="hidden"  value="{{$court}}" name="court" class="btn btn-success" />-->
          <input type="hidden" name="action" value="@if(!is_null($action)){{$action->code}}@endif" />
