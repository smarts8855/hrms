  ...
  <div>
    <div class="col-md-12">
      ...
      function dateDiff($date2, $date1)
     {
      ...
      $days_month = date("t", strtotime("$year1-$mth1-01"));
      // $days_month = 2 ? ($year1 % 4 ? 28 : ($year1 % 100 ? 29 : ($year1 % 400 ? 28 : 29))) : (($mth1 - 1) % 7 % 2 ? 30 : 31);
      $day_diff = 0;
    ...
    //cal_days_in_month(CAL_GREGORIAN, $mth1, $year1);
    $days_month = date("t", strtotime("$year1-$mth1-01"));
    // $days_month =  2 ? ($year1 % 4 ? 28 : ($year1 % 100 ? 29 : ($year1 % 400 ? 28 : 29))) : (($mth1 - 1) % 7 % 2 ? 30 : 31); 
    ...
    $peculiarFG = $oarrears->newPeculiarFG - $oarrears->oldPeculiarFG;
    ...
    $months    = $diff['months'];
    $earnvariation = (($basic + $peculiar + $peculiarFG) * $months) +  (($basic + $peculiar + $peculiarFG) * ($diff['days']/$diff['days_of_month']));
...
$totalBasicArrearsNormal = $earnvariation-$deductvariation;
      ...
      <section>
        <div>
            ...
            <table class="table table-bordered table-condensed" align="right">
              <tr>
                ...
                <tr>
                  ...
                  <div align="left">Names: <strong>{{$fn->surname}} {{$fn->first_name}} {{$fn->othernames}}</strong> <br/>
                  <div align="left">Reason: <strong> {{$oarrearsReason->arrears_type}} </strong> <br/>
              ...
              Division: <strong>{{$oarrearsReason->division}}</strong></div>
            </td>
          ...
          <tr>
            <td>40% PECULIAR FG</td>
            <td align="center">{{number_format($oarrears->oldPeculiarFG, 2, '.', ',')}}</td>
            <td align="center">{{number_format($oarrears->newPeculiarFG, 2, '.', ',')}}</td>
            <td align="center">{{number_format($oarrears->newPeculiarFG - $oarrears->oldPeculiarFG, 2, '.', ',')}}</td>
          </tr>
    ...
    $peculiarFGOverDue  = $overDue->newPeculiarFG - $overDue->oldPeculiarFG;
    ...
    $earnvariationOverDue = (($basicOverDue + $peculiarOverDue + $peculiarFGOverDue + $leaveOverDue + $driverOverDue + $utilityOverDue + $servantOverDue + $furnitureOverDue + $mealOverDue + $transportOverDue + $housingOverDue) * $monthsOverDue) +  (($basicOverDue + $peculiarOverDue + $peculiarFGOverDue + $leaveOverDue + $driverOverDue + $utilityOverDue + $servantOverDue + $furnitureOverDue + $mealOverDue + $transportOverDue + $housingOverDue) * ($diffOverDue['days']/$diffOverDue['days_of_month']));
     ...
     ?>
      ...
      <table class="table table-bordered table-condensed" align="right">
              <tr>
                ...
                <tr>
                  <td colspan="2"><div align="left">File No: <strong>{{$fn->fileNo}} </strong> <br/>
                  <div align="left">Reason: <strong> {{$overDue->arrears_type}} </strong> <br/>
              ...
              Division: <strong>{{$overDue->division}}</strong></div>
            </td>
            <td colspan="2" align="right" valign="bottom"><img src="passport/{{$oarrears->year}}.jpg" width="150" /></td>
          </tr>
          <tr>
          ...
          <tr>
            <td>40% PECULIAR FG</td>
            <td align="center">{{number_format($overDue->oldPeculiarFG, 2, '.', ',')}}</td>
            <td align="center">{{number_format($overDue->newPeculiarFG, 2, '.', ',')}}</td>
            <td align="center">{{number_format($overDue->newPeculiarFG - $overDue->oldPeculiarFG, 2, '.', ',')}}</td>
          </tr>
