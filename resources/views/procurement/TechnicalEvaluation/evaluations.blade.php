...
@section('content')
        ...
        <div class="row">
            ...
            <div class="col-12" style="padding: 10px">
                <div class="panel panel-default" style="border-radius:6px; box-shadow:0 0 5px rgba(0,0,0,0.1);">
                                    ...
                                    <tr data-stat={{ $data->status }} data-recommendation="{{ $data->recommendation }}">
                                        ...
                                        <td>
                                            ...
                                            <!-- <a href={{ '#contract' . $data->contract_biddingID }}
                                                                            ...
                                                                            aria-expanded="false" aria-controls="multiCollapseExample1">view all</a> -->

                                            @if ($data->recommendation == 1)
                                                <a href={{ '#contract' . $data->contract_biddingID }}
                                                    data-target="{{ '#file' . $data->contract_biddingID }}"
                                                    data-toggle="modal" aria-expanded="false"
                                                    aria-controls="multiCollapseExample1" style="color:white;">view all</a>
                                            @else
                                                <a href={{ '#contract' . $data->contract_biddingID }}
                                                    data-target="{{ '#file' . $data->contract_biddingID }}"
                                                    data-toggle="modal" aria-expanded="false"
                                                    aria-controls="multiCollapseExample1">view all</a>
                                            @endif
                                                    ...
                                                    <div class="modal-content">
                                                        ...
                                                        <div class="modal-body">
                                                            @if ($data->status == 0)
                                                                ...
                                                                <form method="POST"
                                                                    action={{ '/procurement/bidding/tech-evaluate/requalify/' . $data->contract_biddingID }}>
                                                                @else
                                                                    <p style="color:grey;">Please give a reason for your
                                                                        Disqualification of <br>
                                                                        {{ $data->company_name . 's Contract' }}</p>
                                                                    <form method="POST"
                                                                        action={{ '/procurement/bidding/tech-evaluate/disqualify/' . $data->contract_biddingID }}>
                                                            @endif
                                                            @csrf
                                                            @method('PUT')
                                                            {{-- <p style="color:grey;">Disqualification checklist</p> --}}
                                                            <div class="row col-12">
                                                                <label class="form-check-label" for="disqualifyTechComment"
                                                                    style="color:grey">Reason</label>
                                                                <textarea name="disqualifyComment" class="form-control" id="disqualifyTechComment"></textarea>
                                                                {{-- @if (isset($checklist) && $checklist)
                                                                        @foreach ($checklist as $key => $checklists)
                                                                            <div class="form-group form-check col-md-4">
                                                                                <input type="checkbox"
                                                                                    value= "{{ $checklists->checklistID }}"
                                                                                    name="{{ 'checklist' . $key }}"
                                                                                    class="form-check-input" id="exampleCheck1">
                                                                                <label class="form-check-label"
                                                                                    for="exampleCheck1"
                                                                                    style="color:grey">{{ $checklists->checklistName }}</label>
                                                        ...
                                                        <div class="modal-footer">
                                                            ...
                                                            @else
                                                                <button type="submit" class="btn btn-danger"
                                                                    id="disqualifyTechButton" disabled>Disqualify
                                                                    Contract</button>
                                                            @endif
                                                            </form>
                                                        </div>
                    ...
                    <button class="btn btn-sm btn-primary recommend_option" data-toggle="modal"
                        value={{ $data->contract_biddingID }} data-recommendation={{ $data->recommendation }}
                        data-target={{ '#recommend' . $data->contract_biddingID }}
                        style="margin-bottom:5px;">Recommend</button>
                    <!-- Modal -->
                            ...
                            <div class="modal-content">
                                ...
                                <div class="modal-body">
                                    <p style="color:grey;">Please give a reason for your Recommendation of <br>
                                        {{ $data->company_name . 's Contract' }}</p>
                                    <form method="POST"
                                        action={{ '/procurement/bidding/tech-evaluate/recommend/' . $data->contract_detailsID }}>
                                        @csrf
                                        @method('PUT')
                                        <div class="form-group">
    ...
    @endif
                ...
                @else
                    <form action="{{ url('/procurement/bidding/tech-evaluate/recommend/reverse/' . $data->contractID) }}"
                        method="post">
                        @csrf
                        @method('PUT')
                        <input type="hidden" value="{{ $data->contract_biddingID }}" name="biddingID">
    ...
    @if ($datas[0]->current_location < 2 && $contract->status == 1)
        <div class="row">
            ...
            {{-- <button style="margin-left:13px;" data-toggle="modal" data-target="#secretary" id="to_secretary"
                    class="btn btn-success">Move to Secretary</button>
                <button id="to_tenders" class="btn btn-success btn-warning" data-toggle="modal"
                    data-target="#tenders">Move to Tenders Board</button>
                <button id="to_tenders" class="btn btn-primary" data-toggle="modal" data-target="#f_tenders">Move To
                    Federal Judiciary Tenders Board</button> --}}
            {{-- <button id="block" class="btn btn-dark" data-toggle="modal" data-target="#blocks">Cancel
                Bids</button> --}}
            <button id="block" class="btn btn-danger btn-sm" data-toggle="modal" data-target="#blocks">
                <i class="fa fa-ban"></i> Cancel Bids
            </button>
            {{-- @else
                <button style="margin-left:13px;" data-toggle="modal" data-target="#warning" id="to_secretary"
                    class="btn btn-success">Move to Secretary</button>
                <button id="to_tenders" class="btn btn-success btn-warning" data-toggle="modal"
                    data-target="#warning">Move to Tenders Board</button>
                <button id="to_tenders" class="btn btn-primary" data-toggle="modal" data-target="#warning">Move To
                    Federal Judiciary Tenders Board</button>
                <button id="block" class="btn btn-dark" data-toggle="modal" data-target="#blocks">Block
                    Bids</button>
            @endif --}}
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <form method="POST" action="/procurement/to-f-tenders/{{ $datas[0]->contractID }}"
                                enctype="multipart/form-data">
                                @csrf
                                <div class="form-group">
                                    <textarea name="comment" class="form-control" placeholder="reason"></textarea>
                ...
                <div class="modal-dialog" role="document">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <form method="POST"
                                action="/procurement/tech-evaluate/to-block/{{ base64_encode($datas[0]->contractID) }}"
                                enctype="multipart/form-data">
                                @csrf
...
@section('scripts')
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            console.log("loaded")
            const textarea = document.getElementById('disqualifyTechComment');
            const submitButton = document.getElementById('disqualifyTechButton');
